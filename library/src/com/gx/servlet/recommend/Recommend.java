package com.gx.servlet.recommend;
...
public class Recommend extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		List<NewBook> listAll=	bookDao.findAll();*/
		 List<Users> pfuser =(List<Users>) req.getSession().getAttribute("user");
		//List<NewBook> list =bookDao.findBorrowAllBookByUserID(pfuser.get(0).getUsersID());
		 BookDao bookDao=new ImplBookDao();
		...
		List<NewBook> list =bookDao.findBorrowAllBookByUserID(pfuser.get(0).getUsersID());
			...
			List<BorrowBook> L=	bookDao.SelectEvaluate(pfuser.get(0).getUsersID());//通过用户ID来查询借书数据，安评价降序
			...
			for (String string : ListStr) 
			{
				...
				int bookID=0;
				 ...
				 for (int i = 0; i <ListLike.size(); i++) 
				 {
					 ...
					 {	
						 System.out.println("1--"+liste.get(j).getUsersID()+",2--"+ListLike.get(i).getUsersID());
			...
			req.setAttribute("List",similarityList);
			req.getRequestDispatcher("/JSP/MyLike.jsp").forward(req, resp);
